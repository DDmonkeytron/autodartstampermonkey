...
function renderLayout(){const L=C.layout||(C.layout={});
 ...
 lo.innerHTML=[num('players'),chk('showAvg'),chk('showLegs'),chk('showThrows'),chk('showCheckout'),chk('autoResetStats'),num('brightness'),
  ...
  +'<br><b>Play light</b> — strips '+sl('playLightMode',['off','playing','always'])
  +' colour <input type=color value="'+hx(L.playLightColor)+'" onchange="lset(\'playLightColor\',rgb(this.value))">'
  +' '+chk('playLightStrobe')
  +'<div class=hint><b>off</b> = strips dark between events &middot; <b>playing</b> = solid colour while a game is on screen &middot; <b>always</b> = also lit on the idle clock. Default bright white; celebrations flash over it then return. <b>playLightStrobe</b> = a very gentle pulse. Brightness follows stripBrightness.</div>'
